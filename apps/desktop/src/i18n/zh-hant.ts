...
export const zhHant = defineLocale({
  ...
  connectionMode: {
    title: '連線',
    description: '選擇此應用程式連線 Hermes 後端的方式。',
    localTitle: '本機執行環境',
    localDesc: '進階：在本機執行私有的 Hermes 後端。可離線使用。',
    clientTitle: '用戶端模式',
    clientDesc: '連線到在另一台機器或伺服器上已在執行的 Hermes 後端。建議使用。',
    currentLocal: '正在使用本機的執行環境。',
    currentRemote: url => `用戶端模式 — 已連線至 ${url}。`,
    urlTitle: '後端網址',
    urlDesc: '遠端儀表板後端的基底網址。支援路徑前綴，例如 /hermes。',
    recentEndpoints: '最近使用的端點',
    connect: '連線',
    useLocal: '使用本機執行環境',
    switchToLocal: '切換到本機執行環境',
    switchToClient: '切換到用戶端模式',
    reconnect: '重新連線',
    advanced: '開啟完整的閘道設定',
    connectedTitle: '已啟用用戶端模式',
    connectedMessage: 'Hermes Desktop 將重新連線至遠端後端。',
    switchedLocalTitle: '已啟用本機執行環境',
    switchedLocalMessage: 'Hermes Desktop 將重新啟動其本機後端。',
    firstRunTitle: '您想如何使用 Hermes？',
    firstRunDescription: '連線到現有的 Hermes 後端，或在本機建立私有的 Hermes 執行環境。',
    firstRunLocalDesc: '進階：在本機安裝並執行完整的 Hermes 後端。可離線使用。',
    firstRunClientDesc: '連線到已在其他位置執行的 Hermes 後端。建議 — 無需在本機安裝。',
    firstRunSetUpLocal: '建立本機 Hermes',
    firstRunLocalToastTitle: '正在建立本機 Hermes',
    firstRunLocalToastMessage: 'Hermes Desktop 將安裝並啟動本機執行環境。',
    insecureCertTitle: '允許自我簽署憑證',
    insecureCertDesc: '略過此閘道的 TLS 憑證檢查。僅在您信任並掌控的伺服器上啟用。'
  },
  ...
  settings: {
    ...
    gateway: {
      ...
      insecureCertTitle: '允許自我簽署憑證',
      insecureCertDesc: '略過此遠端閘道的 TLS 憑證驗證。僅在您掌控且使用自我簽署或其他不受信任憑證的伺服器上使用。',
