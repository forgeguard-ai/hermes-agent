  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        ...
        // Opt-in TLS bypass for a self-signed / untrusted gateway certificate;
        // ignored by http:// requests. When the call site does not decide
        // explicitly, fall back to the per-host opt-in list so upstream call
        // sites (readiness probe, revalidation, profile fetches) inherit the
        // bypass without threading it through every signature. (ForgeGuard fork)
        ...(options.rejectUnauthorized === false ||
        (options.rejectUnauthorized === undefined && hostAllowsInvalidCertificate(parsed.hostname))
          ? { rejectUnauthorized: false }
          : {})
...
function buildRemoteBlock(remoteUrl, authMode, token, allowInvalidCertificate, org?: string) {
  if (authMode !== 'oauth' && !decryptDesktopSecret(token)) {
    throw new Error('Remote gateway session token is required.')
  }

  const block: { url: string; authMode: string; token: object; allowInvalidCertificate: boolean; org?: string } = {
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  if (key) {
    ...
    if (remoteLike) {
      const block = buildRemoteBlock(remoteUrl, authMode, nextToken, allowInvalidCertificate, cloudOrg)
  ...
  const nextRemote = remoteLike
    ? buildRemoteBlock(remoteUrl, authMode, nextToken, allowInvalidCertificate, cloudOrg)
    : existingMode === 'ssh'
      ? rawExistingBlock
      : {
...
async function resolveRemoteBackend(profile) {
  ...
  if (override) {
    ...
    return buildRemoteConnection(
      ...
      undefined,
      override.allowInvalidCertificate
    )
  }
  ...
  return buildRemoteConnection(
    ...
    undefined,
    config.remote?.allowInvalidCertificate
  )
}
