 ...
 *
 * Kept standalone (no `import 'electron'`) so it can be unit-tested — same
 * pattern as connection-config.ts / backend-probes.ts. main.ts owns the
 * electron-coupled signals (fs read/write, env, install markers) and passes
 * plain booleans/values into these helpers.
 *
 ...
 *                            re-prompted.
 *
 * A pure boolean gate: main.ts computes the three signals from the filesystem,
 * env, and connection.json, then calls this.
 */
function firstRunChoiceRequired({ choiceRecorded, hasExplicitRemote, hasExistingLocalInstall }: any = {}) {
