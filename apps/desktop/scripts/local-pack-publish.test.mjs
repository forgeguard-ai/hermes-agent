...
describe('local desktop pack stays out of the publish path', () => {
  ...
  test('publish resolution succeeds with a GitHub token present', async () => {
    ...
    // ForgeGuard fork: the desktop package's repository is the fork, so publish
    // resolution must land on forgeguard-ai — a GitHub-provider config that
    // pointed at upstream would try to publish the fork's installers upstream.
    assert.equal(configs[0].owner, 'forgeguard-ai')
